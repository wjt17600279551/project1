...
BM.Map.TouchExtend = BM.Handler.extend({
	...
	_filterClick: function (e) {
		var timeStamp = (e.timeStamp || e.originalEvent.timeStamp),
			elapsed = BM.DomEvent._lastClick && (timeStamp - BM.DomEvent._lastClick);
		...
		BM.DomEvent._lastClick = timeStamp;
		return true;
	},
...
BM.Marker.Touch = BM.Marker.extend({
	...
	_initInteraction: function () {
		...
		return BM.Marker.prototype._initInteraction.apply(this);
	},
	...
	_initInteractionLegacy: function () {
		...
		BM.DomUtil.addClass(icon, 'leaflet-clickable');
		BM.DomEvent.on(icon, 'click', this._onMouseClick, this);
		BM.DomEvent.on(icon, 'keypress', this._onKeyPress, this);

		for (var i = 0; i < events.length; i++) {
			BM.DomEvent.on(icon, events[i], this._fireMouseEvent, this);
		}

		if (BM.Handler.MarkerDrag) {
